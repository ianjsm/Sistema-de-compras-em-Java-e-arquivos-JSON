...
public class Comprador extends Pessoa {
	...
	public static void finalizarCompra() {
		...
		File fileCarrinho = new File("C:\\Users\\ianjo\\OneDrive\\Área de Trabalho\\POO\\json\\carrinho\\carrinho.json");
		File fileEstoque = new File("C:\\Users\\ianjo\\OneDrive\\Área de Trabalho\\POO\\json\\produtos\\produtos.json");
		String carrinhoJSON = Administrador.lerArquivoProdutos(fileCarrinho);
		String estoqueJSON = Administrador.lerArquivoProdutos(fileEstoque);
		JSONArray listaProdutosCarrinho = new JSONArray(carrinhoJSON);
		JSONArray listaProdutosEstoque = new JSONArray(estoqueJSON);
		for (int i = 0; i < listaProdutosCarrinho.length(); i++) {
	        JSONObject produtoCarrinho = listaProdutosCarrinho.getJSONObject(i);
	        ...
	        for (int j = 0; j < listaProdutosEstoque.length(); j++) {
	            JSONObject produtoEstoque = listaProdutosEstoque.getJSONObject(j);
	            if (produtoCarrinho.getString("nome").equalsIgnoreCase(produtoEstoque.getString("nome"))) {
	                int novaQntEstoque = produtoEstoque.getInt("quantidade") - produtoCarrinho.getInt("quantidade");
	                produtoEstoque.put("quantidade", novaQntEstoque);
	                int qntCarrinho = produtoCarrinho.getInt("quantidade");
	                double valor = produtoEstoque.getDouble("preco");
	                double precoTotalDoProduto = qntCarrinho * valor;
	                valorTotalCompra += precoTotalDoProduto;
	                break;
	            }
	        }
	    }
	    ...
	    try (FileWriter writer = new FileWriter(fileEstoque)) {
	        writer.write(listaProdutosEstoque.toString(4));
	    } catch (IOException e) {
	        e.printStackTrace();
		}
